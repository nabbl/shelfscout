...
export const settingsSchema = z.object({ maxBookAgeYears: z.number().int().min(0).max(10000).nullable().default(null), languages: z.array(readingLanguageSchema).min(1).max(readingLanguages.length).transform(values => [...new Set(values)]).default(['en']), preferences: z.array(preferenceSchema).max(60).default([]), disabled: z.array(z.string().max(100)).max(100).default([]), includeReviews: z.boolean().default(false), rereads: z.boolean().default(false), allowSeries: z.boolean().default(true), hiddenMoods: z.array(z.string().max(80)).max(100).default([]) });
