...
export function rankPool(books: CatalogBook[], profile: Profile, context: RankContext, assessments: Assessment[] = []): RankedCandidate[] {
    ...
    for (const b of books) {
        ...
        for (const [id, match] of matched) {
            ...
            if (match.negative) {
                negative++;
                risks.push(`May conflict with your ${p.origin} preference to ${p.direction} “${p.value}”: catalog says “${match.quote.slice(0, 180)}”.`);
            }
            else
                positive++;
