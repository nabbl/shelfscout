...
import type { CatalogBook, Profile } from './types';
const docSchema = z.object({ key: z.string().regex(/^\/works\/OL\d+W$/), title: z.string().min(1).max(500), author_name: z.array(z.string()).min(1), first_publish_year: z.number().optional(), cover_i: z.number().optional(), isbn: z.array(z.string()).optional(), language: z.array(z.string()).optional(), subject: z.array(z.string()).optional() });
...
async function catalogJson(path: string) { const wait = Math.max(0, nextCatalogRequest - Date.now()); nextCatalogRequest = Math.max(Date.now(), nextCatalogRequest) + 1000; if (wait)
...
export async function searchCatalog(db: ShelfDb, strategy: Strategy, page = 1): Promise<CatalogBook[]> { const params = new URLSearchParams({ q: strategy.query, fields: 'key,title,author_name,first_publish_year,cover_i,isbn,language,subject', limit: '40', page: String(page) }); const raw = await cached(db, `catalog-v2:${hash(params.toString())}`, 86400000, () => catalogJson(`/search.json?${params}`)) as {
    docs?: unknown[];
}; if (!Array.isArray(raw.docs))
    throw new Error('Invalid catalog search response'); return raw.docs.flatMap(d => { const parsed = docSchema.safeParse(d); if (!parsed.success)
    return []; const v = parsed.data; return [{ key: v.key, title: v.title, author: v.author_name[0], year: v.first_publish_year ?? null, isbns: (v.isbn || []).filter(s => /^\d{13}$/.test(s)), language: v.language?.includes('eng') ? 'eng' : v.language?.[0] || 'und', coverUrl: v.cover_i ? `https://covers.openlibrary.org/b/id/${v.cover_i}-L.jpg` : null, subjects: (v.subject || []).slice(0, 40).map(s => s.slice(0, 160)), description: '', series: null, strategies: [strategy.query] }]; }); }
export async function enrichBook(db: ShelfDb, book: CatalogBook): Promise<CatalogBook> { if (!/^\/works\/OL\d+W$/.test(book.key))
    ...
    value?: unknown;
}).value || '') : ''; return { ...book, description: description.slice(0, 5000), series: Array.isArray(raw.series) ? String(raw.series[0] || '') || null : null, subjects: Array.isArray(raw.subjects) ? raw.subjects.filter((s): s is string => typeof s === 'string').slice(0, 40) : book.subjects }; }
