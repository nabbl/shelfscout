...
async function tick() { const job = claim(); if (!job)
    ...
    const { id, force } = JSON.parse(job.payload_json) as {
    ...
    const message = ["recommendation", "ratings"].includes(job.type) && error instanceof Error ? error.message : "Acquisition worker failed. Recheck the saved acquisition.";
    if(!["recommendation", "ratings"].includes(job.type) && job.attempts >= 8) recordStatus(db, JSON.parse(job.payload_json).id, "needs_attention", message);
    const delay = Math.min(300000, 5000 * 2 ** job.attempts);
    db.prepare("UPDATE jobs SET status=?,run_after=?,last_error=?,locked_at=NULL,updated_at=? WHERE id=? AND status='running' AND attempts=?").run(["recommendation", "ratings"].includes(job.type) || job.attempts >= 8 ? "failed" : "queued", new Date(Date.now() + delay).toISOString(), message, new Date().toISOString(), job.id, job.attempts+1);
}
finally {
